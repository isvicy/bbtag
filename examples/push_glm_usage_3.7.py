#!/usr/bin/env python3
"""Render GLM Coding Plan usage in a compact /stats-like layout for 3.7-inch tags (landscape).
...
示例:
    uv run examples/push_glm_usage_3.7.py --preview-only
    uv run examples/push_glm_usage_3.7.py --device EPD-D984FADA
    uv run examples/push_glm_usage_3.7.py --input-json examples/fixtures/glm_quota_sample.json --preview-only
"""
...
DEFAULT_OUTPUT = "glm-usage-3.7inch.png"
...
TOKEN_LABELS = ("5h limit", "1 week limit")
...
def fetch_quota_json(root_url: str, api_key: str, timeout: float) -> dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "push_glm_usage_3.7.py",
...
def build_rows(payload: dict[str, Any], tzinfo) -> list[UsageRow]:
    try:
        parsed = parse_display_rows(
            payload,
            labels=TOKEN_LABELS,
            mcp_stat=lambda remaining, total: f"{remaining}/{total}",
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="把 GLM Coding Plan usage 画成 3.7 寸电子价签样式并推送 (横屏布局)。",
    )
...
def main() -> int:
    ...
    if profile.name != "3.7inch":
        print("❌ 当前脚本只为 3.7 寸横屏布局设计，请使用 push_glm_usage.py", file=sys.stderr)
        return 2
